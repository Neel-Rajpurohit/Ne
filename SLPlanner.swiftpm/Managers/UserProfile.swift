...
struct UserProfile: Codable {
    ...
    var recommendedStudyMinutes: Int { 25 }
    ...
    var schoolStartDate: Date {
        Calendar.current.date(
            bySettingHour: schoolStartHour, minute: schoolStartMinute, second: 0, of: Date())
            ?? Date()
    }
    var schoolEndDate: Date {
        Calendar.current.date(
            bySettingHour: schoolEndHour, minute: schoolEndMinute, second: 0, of: Date()) ?? Date()
    }
    var tuitionStartDate: Date {
        Calendar.current.date(
            bySettingHour: tuitionStartHour, minute: tuitionStartMinute, second: 0, of: Date())
            ?? Date()
    }
    var tuitionEndDate: Date {
        Calendar.current.date(
            bySettingHour: tuitionEndHour, minute: tuitionEndMinute, second: 0, of: Date())
            ?? Date()
    }
    var breakfastDate: Date {
        Calendar.current.date(
            bySettingHour: breakfastHour, minute: breakfastMinute, second: 0, of: Date()) ?? Date()
    }
    var lunchDate: Date {
        Calendar.current.date(bySettingHour: lunchHour, minute: lunchMinute, second: 0, of: Date())
            ?? Date()
    }
    var dinnerDate: Date {
        Calendar.current.date(
            bySettingHour: dinnerHour, minute: dinnerMinute, second: 0, of: Date()) ?? Date()
    }

    static let `default` = UserProfile(
        name: "", age: 16,
        institutionChoice: "School",
...
class ProfileManager: ObservableObject {
    ...
    func completeOnboarding(profile: UserProfile) {
        ...
        StorageManager.shared.updateRunGoal(profile.recommendedRunKM)
