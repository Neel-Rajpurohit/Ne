...
class TaskCompletionManager: ObservableObject {
    ...
    @Published var runReminderMessage: String?
    ...
    func generateDefaultTasks() {
        ...
        tasks.append(
            .create(
                title: "Daily Run Goal", category: .running, goalValue: profile.recommendedRunKM))
        ...
        tasks.append(
            .create(
                title: "Study Session", category: .study,
                goalValue: Double(profile.recommendedStudyMinutes)))
    ...
    func checkHealthTasks() {
        ...
        for i in dailyTaskSet.tasks.indices {
            guard dailyTaskSet.tasks[i].type == .auto && !dailyTaskSet.tasks[i].isCompleted else {
                continue
            }

            switch dailyTaskSet.tasks[i].category {
            case .running:
                let distance = health.todayDistance + WellnessDataStore.shared.today.runKM
        ...
        // 6. Run Checks (Smart Reminders ONLY for Home)
        if dailyTaskSet.tasks.first(where: { $0.category == .running && $0.isCompleted }) != nil {
            runReminderMessage = nil
        } else {
            // Check if it's evening and run hasn't been met
            let hour = Calendar.current.component(.hour, from: Date())
            if hour >= 18 {
                // Evening check
                if let runTask = dailyTaskSet.tasks.first(where: {
                    $0.category == .running && !$0.isCompleted
                }) {
                    let remaining = runTask.goalValue - runTask.currentValue
                    if remaining > 0 {
                        runReminderMessage = String(
                            format: "%.1f KM left to reach your goal! 🏃", remaining)
                    } else {
                        runReminderMessage = nil
                    }
                } else {
    ...
    func checkDailyReset() {
            ...
            !Calendar.current.isDate(lastDate, inSameDayAs: today)
        {
            ...
            // Check if yesterday's goals were NOT met
            if dailyTaskSet.completionRate < 1.0 {
                // Streak decrease/reset logic
                StorageManager.shared.streakData.checkAndResetIfNeeded()
            }

            // New day detected — reset!
            generateDefaultTasks()

            // Refresh notifications for the new day
    ...
    func performanceTrend() -> PerformanceTrend {
        ...
        let older =
            recentRates.suffix(from: min(3, recentRates.count)).reduce(0, +)
            / max(1.0, Double(recentRates.count - 3))
    ...
    func completeExercise() {
        let planner = PlannerEngine.shared
        for i in planner.todayRoutine.blocks.indices {
            if planner.todayRoutine.blocks[i].type == .exercise
                && !planner.todayRoutine.blocks[i].isCompleted
            {
                planner.todayRoutine.blocks[i].isCompleted = true
                planner.objectWillChange.send()
                break
