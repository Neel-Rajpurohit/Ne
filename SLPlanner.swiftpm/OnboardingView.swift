...
struct OnboardingView: View {
    ...
    @State private var institutionChoice = "School"
    @State private var schoolStart =
        Calendar.current.date(bySettingHour: 8, minute: 0, second: 0, of: Date()) ?? Date()
    @State private var schoolEnd =
        Calendar.current.date(bySettingHour: 14, minute: 30, second: 0, of: Date()) ?? Date()
    @State private var hasTuition = false
    @State private var tuitionStart =
        Calendar.current.date(bySettingHour: 16, minute: 0, second: 0, of: Date()) ?? Date()
    @State private var tuitionEnd =
    ...
    private var introSlides: some View {
        VStack(spacing: 30) {
            ...
            TabView(selection: $currentSlide) {
                slideView(
                    icon: "calendar.badge.clock", title: "Organize Your\nStudent Life",
                    subtitle: "Smart timetable that adapts to your schedule",
                    color: AppTheme.studyBlue
                ).tag(0)
                slideView(
                    icon: "timer", title: "Study Smarter\nwith 25-5 Timer",
                    subtitle: "Pomodoro focus sessions with fun break games",
                    color: AppTheme.primaryPurple
                ).tag(1)
                slideView(
                    icon: "heart.fill", title: "Balance Health\nStudy & Growth",
                    subtitle: "Track runs, water, exercise & mental wellness",
            ...
            HStack(spacing: 8) {
                ForEach(0..<3, id: \.self) { i in
                    Circle()
                        ...
                        .frame(
                            width: i == currentSlide ? 10 : 6, height: i == currentSlide ? 10 : 6
                        )
    ...
    private var userDetailForm: some View {
        NavigationStack {
                ...
                VStack(spacing: 20) {
                    ...
                    VStack(alignment: .leading, spacing: 14) {
                        ...
                        HStack {
                            ...
                            Text("Age").font(.system(.subheadline, design: .rounded))
                                .foregroundStyle(AppTheme.textPrimary)
                            Spacer()
                    ...
                    // Schedule
                    VStack(alignment: .leading, spacing: 14) {
                        Text("Your Schedule").font(
                            .system(.headline, design: .rounded, weight: .semibold)
                        ).foregroundStyle(AppTheme.textPrimary)

                        HStack {
                            Image(systemName: "building.columns").foregroundStyle(
                                AppTheme.studyBlue)
                        ...
                        timePicker(
                            ...
                            color: AppTheme.studyBlue, time: $schoolEnd)
                    ...
                    VStack(alignment: .leading, spacing: 14) {
                        Text("Your Daily Goals").font(
                            .system(.headline, design: .rounded, weight: .semibold)
                        ).foregroundStyle(AppTheme.textPrimary)
                        Text("Auto-set based on your age").font(.system(.caption, design: .rounded))
                            .foregroundStyle(AppTheme.textTertiary)

                        let runKM = age <= 17 ? 2.0 : age <= 25 ? 3.0 : age <= 40 ? 2.5 : 2.0
                        let waterML =
                            age <= 8 ? 1300.0 : age <= 13 ? 1800.0 : age <= 18 ? 2300.0 : 2700.0

                        HStack(spacing: 14) {
                            VStack(spacing: 6) {
                                Image(systemName: "figure.run").font(.title2).foregroundStyle(
                                    AppTheme.healthGreen)
                                Text(String(format: "%.1f KM", runKM)).font(
                    ...
                    Button(action: {
                        ...
                        HStack {
                            ...
                            Text("Start My Journey").font(
                                .system(.headline, design: .rounded, weight: .bold))
                        }
                        .foregroundStyle(.white).frame(maxWidth: .infinity).padding(18)
                        .background(AppTheme.studyGradient).clipShape(
            ...
            .alert("Enter Your Name", isPresented: $showNameAlert) {
                ...
                Text("Please scroll up and enter your name to continue.")
            }
        }
    }
