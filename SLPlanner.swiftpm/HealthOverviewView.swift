...
struct HealthOverviewView: View {
    ...
    private var wellnessCard: some View {
        let pct = wellness.today.wellnessPercent
        let completed = wellness.today.completedCount
        ...
        return VStack(spacing: 16) {
            ...
            Text("\(pct)%")
                ...
                .foregroundStyle(
                    pct >= 100 ? AppTheme.healthGreen :
                    pct >= 50 ? AppTheme.warmOrange : .white
                )
            ...
            // 4 mini indicators
            HStack(spacing: 12) {
                wellnessIndicator(icon: "drop.fill", done: wellness.today.waterComplete, color: AppColors.waterColor)
                wellnessIndicator(icon: "figure.run", done: wellness.today.runComplete, color: AppTheme.healthGreen)
                wellnessIndicator(icon: "figure.yoga", done: wellness.today.yogaComplete, color: AppTheme.yogaTeal)
                wellnessIndicator(icon: "wind", done: wellness.today.breathingComplete, color: AppTheme.breathingCyan)
            }
    ...
    private var runningCard: some View {
        let progress = wellness.today.runProgress
        let goal = wellness.today.runGoal
        let current = wellness.today.runKM
    ...
    private var yogaCard: some View {
        ...
        return VStack(alignment: .leading, spacing: 14) {
            ...
            HStack(spacing: 10) {
                quickAddButton(label: "+5 min", color: AppTheme.yogaTeal) {
                    wellness.addYoga(5)
                    HapticManager.impact(.light)
                }
                quickAddButton(label: "+10 min", color: AppTheme.yogaTeal) {
                    wellness.addYoga(10)
                    HapticManager.impact(.light)
                }
    ...
    private var breathingCard: some View {
        ...
        return VStack(alignment: .leading, spacing: 14) {
            ...
            HStack(spacing: 10) {
                quickAddButton(label: "+1 min", color: AppTheme.breathingCyan) {
                    wellness.addBreathing(1)
                    HapticManager.impact(.light)
                }
                ...
                ForEach(data.indices, id: \.self) { idx in
                    VStack(spacing: 6) {
                        Text("\(data[idx])%")
                            ...
                            .foregroundStyle(data[idx] >= 100 ? AppTheme.healthGreen : AppTheme.textSecondary)
                        
                        RoundedRectangle(cornerRadius: 4)
                            .fill(
                                data[idx] >= 100 ? AnyShapeStyle(AppTheme.healthGradient) :
                                data[idx] > 0 ? AnyShapeStyle(AppTheme.neonCyan.opacity(0.4)) :
    ...
    private func quickAddButton(label: String, color: Color, action: @escaping () -> Void) -> some View {
