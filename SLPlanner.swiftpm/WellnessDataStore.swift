...
class WellnessDataStore: ObservableObject {
    ...
    func checkDayReset() {
        ...
        if startOfToday != startOfStored {
            let isRunComplete =
                today.runComplete
                || (today.runKM + HealthKitManager.shared.todayDistance) >= today.runGoal
            let completed = [
                today.waterComplete, isRunComplete, today.yogaComplete, today.breathingComplete,
            ].filter { $0 }.count
            let finalPercent = completed * 25
