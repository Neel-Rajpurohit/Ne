...
struct SettingsView: View {
    ...
    @State private var institutionChoice: String = "School"
    ...
    private let availableCharacters = ["🦊", "🐼", "🐯", "🐰", "🐧", "🦉"]
    ...
    var body: some View {
        ScrollView(showsIndicators: false) {
            VStack(spacing: 20) {
                ...
                VStack(alignment: .leading, spacing: 14) {
                    ...
                    .padding(14).background(Color.white.opacity(0.08)).clipShape(
                        RoundedRectangle(cornerRadius: 12))

                    HStack {
                        Image(systemName: "number").foregroundStyle(AppTheme.neonCyan)
                        Text("Age: \(age)").font(.system(.subheadline, design: .rounded))
                            .foregroundStyle(AppTheme.textPrimary)
                        Spacer()
                    ...
                    VStack(alignment: .leading, spacing: 10) {
                        Text("Profile Photo").font(
                            .system(.subheadline, design: .rounded, weight: .medium)
                        ).foregroundStyle(AppTheme.textSecondary)
                        HStack(spacing: 16) {
                            ZStack {
                                Circle().fill(AppTheme.studyBlue.opacity(0.15)).frame(
                                    width: 60, height: 60)
                                if let uiImage = profileImage {
                                    ...
                                    Text(selectedCharacter).font(.system(size: 32))
                                }
                            }
                            PhotosPicker(
                                "Choose Photo", selection: $selectedPhotoItem, matching: .images
                            )
                            .font(.system(.subheadline, design: .rounded, weight: .semibold))
                            .foregroundStyle(.white)
                            .padding(.horizontal, 16).padding(.vertical, 8)
                            .background(AppTheme.studyBlue)
                            .clipShape(Capsule())
                                ...
                                Task {
                                        ...
                                        let image = UIImage(data: data)
                                    {
                                        if let compressed = image.jpegData(compressionQuality: 0.3)
                                        {
                                            ...
                                            profileManager.profile.profileImageData = compressed
                            ...
                            if profileImage != nil {
                                Button("Clear") {
                                    ...
                                    profileManager.profile.profileImageData = nil
                    ...
                    VStack(alignment: .leading, spacing: 10) {
                        ...
                        ScrollView(.horizontal, showsIndicators: false) {
                            HStack(spacing: 15) {
                                ForEach(availableCharacters, id: \.self) { char in
                                    Button(action: {
                                        selectedCharacter = char
                                        profileManager.profile.selectedCharacter = char
                                        HapticManager.impact(.light)
                                    }) {
                                        Text(char)
                                            .font(.system(size: 32))
                                            ...
                                            .background(
                                                Circle()
                                                    .fill(
                                                        selectedCharacter == char
                                                            ? AppTheme.studyBlue.opacity(0.3)
                                                            : Color.white.opacity(0.05)
                                                    )
                                                    .overlay(
                                                        Circle().stroke(
                                                            selectedCharacter == char
                                                                ? AppTheme.studyBlue : Color.clear,
                                                            lineWidth: 2)
                                                    )
                                            )
                                    }
                ...
                VStack(alignment: .leading, spacing: 14) {
                    ...
                    HStack {
                        Image(systemName: "building.columns").foregroundStyle(AppTheme.studyBlue)
                        Text("Institution Type").font(.system(.subheadline, design: .rounded))
                            .foregroundStyle(AppTheme.textPrimary)
                        Spacer()
                        Picker("Institution", selection: $institutionChoice) {
                            Text("School").tag("School")
                            Text("College").tag("College")
                        }
                        .pickerStyle(.segmented)
                        .frame(width: 150)
                        .onChange(of: institutionChoice) { _ in
                            profileManager.profile.institutionChoice = institutionChoice
                        }
                    }
                    .padding(14).background(Color.white.opacity(0.08)).clipShape(
                        RoundedRectangle(cornerRadius: 12))
                    ...
                    timePicker(
                        label: "\(institutionChoice) Start", icon: "building.columns.fill",
                        color: AppTheme.studyBlue, time: $schoolStart)
                    timePicker(
                        label: "\(institutionChoice) End", icon: "building.columns",
                    ...
                    Toggle(isOn: $hasTuition) {
                        HStack(spacing: 8) {
                            ...
                            Text("Tuition").font(.system(.subheadline, design: .rounded))
                                .foregroundStyle(AppTheme.textPrimary)
                        }
                    }
                    ...
                    if hasTuition {
                        timePicker(
                            label: "Tuition Start", icon: "clock.fill",
                            color: AppTheme.primaryPurple, time: $tuitionStart
                        )
                        .onChange(of: tuitionStart) { _ in
                            let comps = Calendar.current.dateComponents(
                                [.hour, .minute], from: tuitionStart)
                            profileManager.profile.tuitionStartHour = comps.hour ?? 16
                            profileManager.profile.tuitionStartMinute = comps.minute ?? 0
                        }
                        timePicker(
                            label: "Tuition End", icon: "clock", color: AppTheme.primaryPurple,
                            time: $tuitionEnd
                        )
                        .onChange(of: tuitionEnd) { _ in
                            let comps = Calendar.current.dateComponents(
                                [.hour, .minute], from: tuitionEnd)
                    ...
                    VStack(alignment: .leading, spacing: 6) {
                        Text("Meal Times").font(
                            .system(.subheadline, design: .rounded, weight: .medium)
                        ).foregroundStyle(AppTheme.warmOrange)
                    }
                    timePicker(
                        label: "Breakfast", icon: "sunrise.fill", color: AppTheme.warmOrange,
                        time: $breakfastTime)
                    timePicker(
                        label: "Lunch", icon: "fork.knife", color: AppTheme.warmOrange,
                        time: $lunchTime)
                    timePicker(
                        label: "Dinner", icon: "moon.fill", color: AppTheme.warmOrange,
                        time: $dinnerTime)
                }
                .padding(20).glassBackground()
                .opacity(appeared ? 1 : 0)
                ...
                VStack(alignment: .leading, spacing: 12) {
                    ...
                    HStack {
                        TextField("Add subject...", text: $subjectInput)
                            .font(.system(.subheadline, design: .rounded))
                            .foregroundStyle(AppTheme.textPrimary)
                            .padding(12).background(Color.white.opacity(0.08)).clipShape(
                                RoundedRectangle(cornerRadius: 10))
                        Button(action: addSubject) {
                            Image(systemName: "plus.circle.fill").font(.title2).foregroundStyle(
                                AppTheme.studyGradient)
                        }
                    }
                ...
                VStack(alignment: .leading, spacing: 14) {
                    ...
                    Text("Auto-set based on age (\(profileManager.profile.age))").font(
                        .system(.caption, design: .rounded)
                    ).foregroundStyle(AppTheme.textTertiary)

                    HStack(spacing: 14) {
                        VStack(spacing: 6) {
                            Image(systemName: "figure.run").font(.title2).foregroundStyle(
                                AppTheme.healthGreen)
                            Text(String(format: "%.1f KM", profileManager.profile.recommendedRunKM))
                                .font(
                                    .system(.headline, design: .rounded, weight: .bold)
                                ).foregroundStyle(AppTheme.textPrimary)
                            Text("Daily Run Goal").font(.system(.caption2, design: .rounded))
                                .foregroundStyle(AppTheme.textSecondary)
                        }
                        .frame(maxWidth: .infinity).padding(14)
                        .background(AppTheme.healthGreen.opacity(0.1)).clipShape(
                            RoundedRectangle(cornerRadius: 14))

                        VStack(spacing: 6) {
                            Image(systemName: "drop.fill").font(.title2).foregroundStyle(
                                AppTheme.neonCyan)
                            Text(profileManager.profile.waterGoal.asLiters).font(
                                .system(.headline, design: .rounded, weight: .bold)
                            ).foregroundStyle(AppTheme.textPrimary)
                            Text("Water").font(.system(.caption2, design: .rounded))
                                .foregroundStyle(AppTheme.textSecondary)
                        }
                        .frame(maxWidth: .infinity).padding(14)
                        .background(AppTheme.neonCyan.opacity(0.1)).clipShape(
                            RoundedRectangle(cornerRadius: 14))
                    }
                }
                .padding(20).glassBackground()
                ...
                VStack(alignment: .leading, spacing: 12) {
                    ...
                    HStack {
                        Text("Version").font(.system(.subheadline, design: .rounded))
                            .foregroundStyle(AppTheme.textPrimary)
                        Spacer()
                        Text("2.0").font(.system(.subheadline, design: .rounded, weight: .medium))
                            .foregroundStyle(AppTheme.textTertiary)
                    }
                }
                .padding(20).glassBackground()
                    ...
                    Button(action: {
                        ...
                        .background(
                            RoundedRectangle(cornerRadius: 14)
                                ...
                                .background(
                                    AppTheme.neonCyan.opacity(0.05).clipShape(
                                        RoundedRectangle(cornerRadius: 14)))
                        )
                    }
                    .buttonStyle(ScaleButtonStyle())
        ...
        .alert("Restart App?", isPresented: $showRestartAlert) {
            ...
            Text(
                "This will clear your study data, calendar events, XP, and streak. You will start from the beginning."
            )
        }
        .onAppear {
            loadProfile()
    ...
    private func timePicker(label: String, icon: String, color: Color, time: Binding<Date>)
        -> some View
    {
        HStack {
            Image(systemName: icon).foregroundStyle(color)
            Text(label).font(.system(.subheadline, design: .rounded)).foregroundStyle(
                AppTheme.textPrimary)
            Spacer()
            DatePicker("", selection: time, displayedComponents: .hourAndMinute).labelsHidden()
                .colorScheme(.dark).tint(color)
        }
        .padding(14).background(Color.white.opacity(0.08)).clipShape(
            RoundedRectangle(cornerRadius: 12))
    }
    ...
    private func loadProfile() {
        ...
        name = p.name
        age = p.age
        institutionChoice = p.institutionChoice ?? "School"
        selectedCharacter = p.selectedCharacter
        if let data = p.profileImageData, let uiImage = UIImage(data: data) {
        ...
        subjects = p.subjects
        schoolStart = p.schoolStartDate
        schoolEnd = p.schoolEndDate
    }
    ...
    private var restartOptionsSheet: some View {
        NavigationStack {
            VStack(spacing: 20) {
                ...
                Text("Select what to clear. Health distance from Apple won't be deleted.")
                ...
                VStack(spacing: 12) {
                    restartToggle(
                        icon: "book.fill", label: "Study Data", sublabel: "Sessions, study time",
                        color: AppTheme.studyBlue, isOn: $restartStudy)
                    restartToggle(
                        icon: "star.fill", label: "XP, Level & Streak",
                        sublabel: "XP, level, streak, brain score", color: AppTheme.warmOrange,
                        isOn: $restartHealthXP)
                    restartToggle(
                        icon: "calendar", label: "Calendar & Events",
                        sublabel: "All events and reminders", color: AppTheme.quizPink,
                        isOn: $restartCalendar)

                    Divider().overlay(Color.white.opacity(0.1)).padding(.vertical, 4)

                    restartToggle(
                        icon: "trash.fill", label: "Restart Everything",
                        sublabel: "All of the above + profile", color: AppTheme.neonCyan,
                        isOn: $restartEverything
                    )
                ...
                Button(action: {
                    ...
                    .padding(.vertical, 16)
                    .background(
                        (restartStudy || restartHealthXP || restartCalendar)
                            ? AppTheme.neonCyan : Color.gray.opacity(0.3)
                    )
    ...
    private func restartToggle(
        icon: String, label: String, sublabel: String, color: Color, isOn: Binding<Bool>
    ) -> some View {
        HStack(spacing: 12) {
            ...
            VStack(alignment: .leading, spacing: 2) {
                Text(label).font(.system(.subheadline, design: .rounded, weight: .semibold))
                    .foregroundStyle(AppTheme.textPrimary)
                Text(sublabel).font(.system(.caption2, design: .rounded)).foregroundStyle(
    ...
    private func performRestart() {
        ...
        if restartHealthXP {
            ...
            // Set restart date so HealthKit distance only counts from now
