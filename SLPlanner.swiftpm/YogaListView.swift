        ...
        ScrollView(showsIndicators: false) {
            VStack(spacing: 24) {
                ...
                ZStack {
                    ...
                    Image(systemName: yoga.icon).font(.system(size: 55)).foregroundStyle(
                        AppTheme.yogaGradient)
                }
                ...
                VStack(spacing: 6) {
                    ...
                    HStack(spacing: 2) {
                        ...
                        Text(yoga.difficulty.rawValue).font(.system(.caption2, design: .rounded))
                            .foregroundStyle(yoga.difficulty.color)
                    }
                }
                ...
                HStack(spacing: 8) {
                    ForEach(yoga.benefits, id: \.self) { benefit in
                        Text(benefit).font(.system(.caption, design: .rounded, weight: .medium))
                            .foregroundStyle(AppTheme.yogaTeal)
                            .padding(.horizontal, 12).padding(.vertical, 6).background(
                                AppTheme.yogaTeal.opacity(0.15)
                            ).clipShape(Capsule())
                    }
                }
                ...
                VStack(alignment: .leading, spacing: 10) {
                    ...
                    Text(yoga.instructions).font(.system(.subheadline, design: .rounded))
                ...
                VStack(alignment: .leading, spacing: 10) {
                    ...
                    ForEach(yoga.correctForm, id: \.self) { tip in
                        HStack(alignment: .top, spacing: 8) {
                            ...
                            Text(tip).font(.system(.subheadline, design: .rounded)).foregroundStyle(
                    ...
                    VStack(alignment: .leading, spacing: 10) {
                        ...
                        ForEach(yoga.contraindications, id: \.self) { item in
                            HStack(alignment: .top, spacing: 8) {
                                ...
                                Text(item).font(.system(.subheadline, design: .rounded))
                                    .foregroundStyle(AppTheme.textSecondary)
                            }
                        }
                    }
                ...
                if isHolding && !completed {
                    VStack(spacing: 16) {
                        ...
                        Text("Hold the pose").font(.system(.headline, design: .rounded))
                            .foregroundStyle(AppTheme.textSecondary)
                        CircularProgressView(
                            progress: timer.progress, lineWidth: 14, size: 160,
                            gradient: AppTheme.yogaGradient, label: timer.formattedTime,
                            sublabel: "remaining")
                    }
                    .padding(20).glassBackground()
                    .onChange(of: timer.isCompleted) { _ in
                        if timer.isCompleted {
                            ...
                            GameEngineManager.shared.awardXP(
                                amount: GameEngineManager.xpYogaComplete, source: yoga.name,
                                icon: "figure.yoga")
                            TaskCompletionManager.shared.completeExercise()

                            // Add exact time spent holding the pose to WellnessDataStore.
                            let exactMinutes = Double(yoga.holdTime) / 60.0
                            WellnessDataStore.shared.addYoga(exactMinutes)
                        }
                    }
                } else if completed {
                    VStack(spacing: 12) {
                        Image(systemName: "sparkles").font(.system(size: 50)).foregroundStyle(
                            AppTheme.yogaGradient)
                        Text("Pose Complete!").font(
                            .system(.title3, design: .rounded, weight: .bold)
                        ).foregroundStyle(AppTheme.textPrimary)
                        Text("+\(GameEngineManager.xpYogaComplete) XP").font(
                    ...
                    Button(action: {
                        isHolding = true
                        timer.start(duration: yoga.holdTime)
                        HapticManager.impact(.medium)
                    }) {
                        HStack(spacing: 10) {
                            Image(systemName: "play.fill")
                            Text("Start Hold (\(yoga.holdTime)s)").font(
                                .system(.headline, design: .rounded, weight: .bold))
                        }
                        .foregroundStyle(.white).frame(maxWidth: .infinity).padding(18)
                        .background(AppTheme.yogaGradient).clipShape(
