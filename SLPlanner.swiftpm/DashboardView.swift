...
struct DashboardView: View {
            ...
            ScrollView(showsIndicators: false) {
                VStack(spacing: 20) {
                    ...
                    VStack(alignment: .leading, spacing: 6) {
                        ...
                        Text(
                            "Hey \(profileManager.profile.name.isEmpty ? "Student" : profileManager.profile.name)! 👋"
                        )
                    ...
                    // Smart Step Reminder Banner
                    if let reminder = taskManager.runReminderMessage {
                        HStack(spacing: 12) {
                        ...
                        .background(
                            LinearGradient(
                                colors: [AppTheme.warmOrange, Color(hex: "EF4444")],
                        ...
                        HStack(spacing: 16) {
                            ...
                            VStack(alignment: .leading, spacing: 4) {
                                Text("Daily Tasks")
                                    ...
                                    .foregroundStyle(AppTheme.textPrimary)
                                Text(
                                    "\(taskManager.dailyTaskSet.completedCount)/\(taskManager.dailyTaskSet.totalCount) completed"
                                )
                                .font(.system(.caption, design: .rounded))
                                .foregroundStyle(AppTheme.textSecondary)
                            }
                    ...
                    {
                        quickCard(
                            icon: "book.fill", label: "Study",
                            value: "\(planner.todayRoutine.studyBlocks.count) sessions",
                            color: AppTheme.studyBlue, gradient: AppTheme.studyGradient)
                        quickCard(
                            icon: "drop.fill", label: "Water",
                            value: storage.todayWater.totalML.asLiters, color: AppColors.waterColor,
                            gradient: AppColors.waterGradient)
                        quickCard(
                            icon: "figure.run",
                            label: "Daily Run Goal",
                            value: String(
                                format: "%.1f KM", healthKit.todayDistance + wellness.today.runKM),
                            color: AppColors.runColor,
                            gradient: AppColors.runGradient)
                        NavigationLink(destination: QuizHomeView()) {
                            quickCard(
                                icon: "brain.head.profile", label: "Quiz",
                                value: "\(QuizManager.shared.totalQuizzesTaken) done",
                                color: AppTheme.quizPink, gradient: AppTheme.quizGradient)
                        }
                        ...
                        ForEach(planner.todayRoutine.blocks.prefix(5)) { block in
                            HStack(spacing: 12) {
                                if block.isCompleted {
                                    ...
                                    RoundedRectangle(cornerRadius: 2).fill(block.type.lightColor)
                                        .frame(width: 4, height: 30)
                                }
                                Image(
                                    systemName: block.isCompleted
                                        ? "checkmark.circle.fill" : block.type.icon
                                )
                                .font(.caption)
                                .foregroundStyle(
                                    block.isCompleted ? AppTheme.healthGreen : block.type.lightColor
                                )
                                VStack(alignment: .leading, spacing: 1) {
                                    Text(block.displayTitle)
                                        ...
                                        .foregroundStyle(
                                            block.isCompleted
                                                ? AppTheme.textTertiary : AppTheme.textPrimary
                                        )
                                        .strikethrough(
                                            block.isCompleted, color: AppTheme.textTertiary)
                    ...
                    if brainScore.todayScore.gamesPlayed > 0 {
                        VStack(alignment: .leading, spacing: 12) {
                            ...
                            HStack(spacing: 8) {
                                brainStatPill(
                                    icon: "rectangle.on.rectangle.angled", label: "Memory",
                                    value: brainScore.todayScore.memoryPoints,
                                    color: AppTheme.primaryPurple)
                                brainStatPill(
                                    icon: "square.grid.3x3.fill", label: "Speed",
                                    value: brainScore.todayScore.speedPoints,
                    ...
                    HStack(spacing: 12) {
                        ...
                        Text(
                            GreetingHelper.motivationalMessage(
                                runProgress: profileManager.profile.recommendedRunKM > 0
                                    ? (healthKit.todayDistance + wellness.today.runKM)
                                        / profileManager.profile.recommendedRunKM : 0.0,
                                waterProgress: storage.todayWater.totalML
                                    / profileManager.profile.waterGoal
                            )
                        )
                        .font(.system(.caption, design: .rounded))
                        .foregroundStyle(AppTheme.textSecondary)
    ...
    private var studyProgress: Double {
        ...
        return min(
            1.0,
            Double(game.recentXP.filter { Calendar.current.isDateInToday($0.timestamp) }.count)
                / Double(planner.todayRoutine.studyBlocks.count))
    }

    private func quickCard(
        icon: String, label: String, value: String, color: Color, gradient: LinearGradient
    ) -> some View {
        VStack(spacing: 10) {
            Image(systemName: icon).font(.title2).foregroundStyle(gradient)
            Text(value).font(.system(.headline, design: .rounded, weight: .bold)).foregroundStyle(
                AppTheme.textPrimary)
            Text(label).font(.system(.caption, design: .rounded)).foregroundStyle(
    ...
    private func brainStatPill(icon: String, label: String, value: Int, color: Color) -> some View {
        VStack(spacing: 6) {
            ...
            Text("\(value)").font(.system(.subheadline, design: .rounded, weight: .bold))
                .foregroundStyle(AppTheme.textPrimary)
            Text(label).font(.system(.caption2, design: .rounded)).foregroundStyle(
                AppTheme.textSecondary)
        }
