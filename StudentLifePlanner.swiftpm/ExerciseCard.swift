...
struct ExerciseCard: View {
    ...
    var body: some View {
        HStack(spacing: 14) {
            ZStack {
                Circle().fill(AppTheme.warmOrange.opacity(0.15)).frame(width: 50, height: 50)
                Image(systemName: exercise.icon).font(.title3).foregroundStyle(AppTheme.fitnessGradient)
            }
            ...
            VStack(alignment: .leading, spacing: 4) {
                ...
                HStack(spacing: 8) {
                    ...
                    Text(exercise.muscleGroup).font(.system(.caption2, design: .rounded)).foregroundStyle(AppTheme.warmOrange.opacity(0.8))
                }
            }
        ...
        HStack(spacing: 14) {
            ...
            VStack(alignment: .leading, spacing: 4) {
                ...
                HStack(spacing: 8) {
                    ...
                    Text(yoga.benefits.joined(separator: " • ")).font(.system(.caption2, design: .rounded)).foregroundStyle(AppTheme.yogaTeal.opacity(0.8)).lineLimit(1)
                }
            }
