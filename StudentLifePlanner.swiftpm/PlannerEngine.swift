...
class PlannerEngine: ObservableObject {
    ...
    func generateRoutine(for date: Date, profile: UserProfile) -> DailyRoutine {
        ...
        // 1. Wake Up — 1 hour before school
        let wakeHour = profile.schoolStartHour - 1
        blocks.append(TimeBlock(type: .wakeUp, startHour: max(5, wakeHour), startMinute: 0, endHour: profile.schoolStartHour, endMinute: profile.schoolStartMinute))
        ...
        let studyDuration = profile.recommendedStudyMinutes
        let breakDuration = profile.recommendedBreakMinutes
        ...
        // Create study blocks for each subject (max 3 subjects per day)
        let dailySubjects = Array(profile.subjects.prefix(3))
        for subject in dailySubjects {
            // Study block
            let studyEnd = advanceTime(hour: studyStartH, minute: studyStartM, byMinutes: studyDuration)
            blocks.append(TimeBlock(type: .study, subject: subject, startHour: studyStartH, startMinute: studyStartM, endHour: studyEnd.h, endMinute: studyEnd.m))
            
            // Break block
            let breakEnd = advanceTime(hour: studyEnd.h, minute: studyEnd.m, byMinutes: breakDuration)
            blocks.append(TimeBlock(type: .breakTime, startHour: studyEnd.h, startMinute: studyEnd.m, endHour: breakEnd.h, endMinute: breakEnd.m))
            
            studyStartH = breakEnd.h
            studyStartM = breakEnd.m
        ...
        // 7. Exercise — 30 min
        if studyStartH < 21 {
            let exerciseEnd = advanceTime(hour: studyStartH, minute: studyStartM, byMinutes: 30)
            blocks.append(TimeBlock(type: .exercise, startHour: studyStartH, startMinute: studyStartM, endHour: exerciseEnd.h, endMinute: exerciseEnd.m))
        }
